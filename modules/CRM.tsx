  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="relative">
          <input 
            ...
            className="bg-[#111] border border-white/10 rounded-2xl px-10 py-3 text-sm text-gray-200 w-80 focus:border-amber-500 outline-none"
          />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.map(c => (
          <div key={c.id} className="bg-[#111] border border-white/5 rounded-3xl p-6 group hover:border-amber-500/30 transition-all">
            <div className="flex items-start justify-between mb-4">
              <div className="h-14 w-14 rounded-2xl bg-gradient-to-br from-gray-800 to-black flex items-center justify-center text-xl font-bold text-amber-500">
                {c.name[0]}
              </div>
              {c.isVIP && (
                <span className="bg-amber-500/10 text-amber-500 text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded">VIP Client</span>
              )}
            </div>
